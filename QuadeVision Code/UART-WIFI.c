...
void config_UARTW(){
    /*
     * This function configures UART over GPIO pins, tested using the AD2, THIS IS WHERE THE WIFI MODULE WOULD BE CONNECTED IF IMPLEMENTED
     * Pins 3.3 and 3.2 use EUSCI_A2
     * */
...
//    CS->KEY = 0;                            // Lock CS module from unintended accesses //THIS SECTION CONFIGURED IN UART.c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//            EUSCI_A_MCTLW_OS16;
    ...
    EUSCI_A2->BRW = 6;                     // 12000000/16/115200
    EUSCI_A2->MCTLW = (8 << EUSCI_A_MCTLW_BRF_OFS) |//was2
            EUSCI_A_MCTLW_OS16;
...
void uart_outW(const char *str){ //Adapted from source in main.c
    if (str != '\0') { //IF the buffer is not empty
        ...
        while (*str != '\0') { //runs through all of the characters in the buffer
            ...
            if (*str == '\n') {
                /* Wait for the transmit buffer to be ready */
                while (!(EUSCI_A2->IFG & EUSCI_A_IFG_TXIFG));
                EUSCI_A2->TXBUF = '\r'; //Goes to the next line
            }
