...
extern int miliseconds;
 ...
 static void Delay(uint32_t loop) //1 = delay 10us
{
    volatile uint32_t i;
...
void US_echo(){
    ...
    Delay(3000);             // delay for 30ms (after this time echo times out if there is no object detected) Allows the P5.2 interrupt to run
